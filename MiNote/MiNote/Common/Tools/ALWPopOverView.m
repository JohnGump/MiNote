...
- (void)showFrom:(UIView *)from alignStyle:(ALWAlingStyle)style
{
    ...
    if ([self.delegate respondsToSelector:@selector(popPverViewDidShow:)]) {
        [self.delegate popPverViewDidShow:self];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(popOverView:didClickedMenuIndex:)]) {
        [self.delegate popOverView:self didClickedMenuIndex:indexPath.row];
    }
}
